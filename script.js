    ...
    if (!slot.classList.contains("taken")) {
      ...
      slot.addEventListener("mouseenter", () => {
        if (oneHourMode) {
          ...
          const valid = preview.length === 3 && preview.every(s => !s.classList.contains("taken"));
          if (valid) preview.forEach(s => s.classList.add("hovered-1h"));
        }
      });
...
function generateSlotTable(reservations) {
  ...
  for (let h = 17; h <= 23; h++) {
    ...
    [0, 20, 40].forEach(min => {
      ...
      if (reservations && reservations[dateStr] && reservations[dateStr][time]) {
        slot.classList.add("taken");
        if (isAdmin) {
          slot.title = `Jméno: ${reservations[dateStr][time].name}\nTel: ${reservations[dateStr][time].phone}`;
  ...
  // zbytek původní funkce pokračuje dál...
}
