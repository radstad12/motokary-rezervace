...
let forbiddenTimes = []; // nastaví admin

document.getElementById("statsBtn").addEventListener("click", () => {
  alert("Statistiky (demo):\nPočet rezervací dnes: 12\nTop čas: 20:40");
});
...
// Rozšíření slotů
const originalGenerate = generateSlotTable;
generateSlotTable = function (reservations) {
  originalGenerate(reservations);

  const now = new Date();
  const currentTime = now.getHours() * 60 + now.getMinutes();
  const today = formatDate(now);
  const currentIsToday = formatDate(selectedDate) === today;
  ...
  document.querySelectorAll(".slot").forEach(slot => {
    ...
    if (forbiddenTimes.includes(timeStr)) {
      slot.classList.add("forbidden");
      slot.title = "Není možné rezervovat";
      slot.onclick = (e) => e.stopPropagation();
    }

    if (currentIsToday && timeMinutes < currentTime) {
      slot.classList.add("forbidden");
      slot.title = "Čas již minul";
    }

    if (!slot.classList.contains("taken") && !slot.classList.contains("forbidden")) {
      slot.addEventListener("click", () => {
        slot.classList.add("clicked");
        showConfirmation("Rezervace potvrzena");
      });
    }
  });
...
// Po přihlášení admina se zobrazí tlačítko statistik
document.getElementById("adminBtn").addEventListener("click", () => {
  setTimeout(() => {
